...
function App() {
  ...
  const ClickFunction = () => {
    useEffect(()=>{
      const getGreeting2 = async()=>{ //to be able to use async and await in useEffect we need to create an async function.
        const response = await fetch("http://localhost:5001/newcollection");//we fetch from our api server running on port 5001
        const data2 = await response.json(); //we get the json data
        //the data is an array with one item. This item is an object with _id and greeting properties
        //this makes sense since we turned the data into an array in line 20 in server.js and the array is the collection from MongoDB
        //this collection has only one document and our objet represents the data in that document.
  
        //now let's get the string from greeting into our greeting state:
        setGreeting2(data2[0].paragraph);
      }
  ...
  return (
    <>
      ...
      <div>
        <h2 onClick={ClickFunction}><a href="/newcollection">Click here</a></h2>
      </div>
      <div>
        <h2>{greeting2}</h2>
      </div>
